...
def encoder(X, keep_prob):
    ...
    act = conv_layer(X, ch_in=3, ch_out=128, kernel_size=5, stride=2, padding="SAME", activation_function="relu",
                     name="conv1")
    act = tf.contrib.layers.batch_norm(act)
    act = conv_layer(act, ch_in=128, ch_out=256, kernel_size=5, stride=2, padding="SAME", activation_function="relu",
                     name="conv2")
    act = tf.contrib.layers.batch_norm(act)
    act = conv_layer(act, ch_in=256, ch_out=512, kernel_size=5, stride=2, padding="SAME", activation_function="relu",
                     name="conv3")
    act = tf.contrib.layers.batch_norm(act)
    act = conv_layer(act, ch_in=512, ch_out=1024, kernel_size=4, stride=2, padding="SAME", activation_function="relu",
                     name="conv4")
    act = tf.contrib.layers.batch_norm(act)

    # TODO in_size
    act = fc_layer(tf.contrib.layers.flatten(act), in_size= , out_size=64, activation_function=None)
...
def decoder(X):
    ...
    reshaped = tf.reshape(X, shape=[X.shape[0],1,1,X.shape[1]])
    act = tf.image.reshape_image(reshaped, [4, 4])
