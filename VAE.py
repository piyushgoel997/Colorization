...
def encoder(X, d, keep_prob, parameters):
    Z = tf.nn.conv2d(X, parameters['W1'], [1, 2, 2, 1],
                     padding='SAME')  # + parameters['b1'] #kernel = 5*5, strides = 2, o/p channels = 128
    A = tf.nn.relu(Z)
    A = tf.contrib.layers.batch_norm(A)

    Z = tf.nn.conv2d(A, parameters['W2'], [1, 2, 2, 1],
                     padding='SAME')  # + parameters['b2'] #kernel = 5*5, strides = 2, o/p channels = 256
    A = tf.nn.relu(Z)
    A = tf.contrib.layers.batch_norm(A)

    Z = tf.nn.conv2d(A, parameters['W3'], [1, 2, 2, 1],
                     padding='SAME')  # + parameters['b3'] #kernel = 5*5, strides = 2, o/p channels = 512
    A = tf.nn.relu(Z)
    A = tf.contrib.layers.batch_norm(A)

    Z = tf.nn.conv2d(A, parameters['W4'], [1, 2, 2, 1],
                     padding='SAME')  # + parameters['b4'] #kernel = 4*4, strides = 2, o/p channels = 1024
    A = tf.nn.relu(Z)
    A = tf.contrib.layers.batch_norm(A)
...
def decoder(d, parameters):
    ...
    Z = tf.nn.conv2d(A, parameters['W5'], [1, 1, 1, 1],
                     padding='SAME')  # + parameters['b5']  # kernel = 4*4, strides = 1, o/p channels = 1024
    A = tf.nn.relu(Z)
    A = tf.contrib.layers.batch_norm(A)

    A = tf.image.resize_images(A, [8, 8])

    Z = tf.nn.conv2d(A, parameters['W6'], [1, 1, 1, 1],
                     padding='SAME')  # + parameters['b6']  # kernel = 5*5, strides = 1, o/p channels = 512
    A = tf.nn.relu(Z)
    A = tf.contrib.layers.batch_norm(A)

    A = tf.image.resize_images(A, [16, 16])

    Z = tf.nn.conv2d(A, parameters['W7'], [1, 1, 1, 1],
                     padding='SAME')  # + parameters['b7']  # kernel = 5*5, strides = 1, o/p channels = 256
    A = tf.nn.relu(Z)
    A = tf.contrib.layers.batch_norm(A)

    A = tf.image.resize_images(A, [32, 32])

    Z = tf.nn.conv2d(A, parameters['W8'], [1, 1, 1, 1],
                     padding='SAME')  # + parameters['b8']  # kernel = 5*5, strides = 1, o/p channels = 128
    A = tf.nn.relu(Z)
    A = tf.contrib.layers.batch_norm(A)

    A = tf.image.resize_images(A, [64, 64])
    Z = tf.nn.conv2d(A, parameters['W9'], [1, 1, 1, 1],
                     padding='SAME')  # + parameters['b9']  # kernel = 5*5, strides = 1, o/p channels = 2
    A = tf.tanh(Z)
    A = tf.contrib.layers.batch_norm(A)
    ...
    with  tf.Session() as sess:
        ...
        for epoch in range(num_epochs):
            ...
            for minibatch in minibatches:
                ...
                print("epoch no = " + str(epoch) + ", minibatch = " + str(i) + "/" + str(
                    len(minibatches)) + ", loss = " + str(temp_cost) + " in " + str(mb_time) + "secs")
                i += 1
